...
- Never commit secrets (`.dev.vars`, API keys, Clerk secrets). Store values in 1Password Environments only; the repo lists variable names in `*.example` manifests. Deploy uploads secrets via `op run` and `wrangler deploy --secrets-file` — do not author secrets with `wrangler secret put` or the Cloudflare dashboard.

...
- For a signed-in browser session: `pnpm run dev` (and `dev:setup` once), then open `http://localhost:5190/dev/agent-signin`. It signs in as `AGENT_LOGIN_EMAIL` and lands on `/dashboard`; open it again if you get signed out. No token or password passes through you, and the browser never leaves localhost, so it works where Clerk's own domain is blocked. It 404s unless `APP_ENV=development`, the Clerk key is `sk_test_`, and the browser connects straight to the dev server from this machine (not over the LAN or through a tunnel).

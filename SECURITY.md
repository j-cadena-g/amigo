...
- Cursor Cloud Agents should use the same bootstrap as Workers Builds, with prefixed Cursor secret names: only `AMIGO_OP_SERVICE_ACCOUNT_TOKEN` and `AMIGO_OP_ENVIRONMENT_ID` in Cursor environment secrets, pointed at **your** local-dev Environment — not copies of individual app keys. Put agent UI login credentials in that Environment as `AGENT_LOGIN_EMAIL` / `AGENT_LOGIN_PASSWORD` (see README), never in the repo.

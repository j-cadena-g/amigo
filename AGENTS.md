...
| Sign the browser in as the agent | open `http://localhost:5190/dev/agent-signin` |
| Lint | `pnpm run lint` |
...
- For a signed-in browser session: `pnpm run dev` (and `dev:setup` once), then open `http://localhost:5190/dev/agent-signin`. It signs in as `AGENT_LOGIN_EMAIL` and lands on `/dashboard`; open it again if you get signed out. You never copy a token or password: the dev server mints a one-time token and redirects the browser to `/?__clerk_ticket=…`, and that URL is still a credential, so don't paste it into logs, PRs, or chat. The browser never leaves localhost, so it works where Clerk's own domain is blocked. It 404s unless `APP_ENV=development`, the Clerk key is `sk_test_`, and the request comes straight from this machine: not over the LAN, not through a proxy or HTTP tunnel, and not marked `Sec-Fetch-Site: cross-site` by the browser.
- A raw TCP forward (`ssh -R`, `socat`) looks like a local connection to the dev server, so don't expose the dev server that way while `AGENT_LOGIN_EMAIL` is set.
- `pnpm run agent:signin-url` prints a one-time Clerk-hosted sign-in URL instead, for browsers that can open Clerk's domain. Add `--ticket` to skip Clerk's domain and get a localhost URL; it expires in 120s, so open it right away. stdout is the URL only — do not paste it into logs, PRs, or chat.
